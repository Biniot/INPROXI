...
*/
@Injectable()
export class RoomMessageStorageProvider {
  // Hold idRoomKey
  keyList: any;
  // Hold message by idRoomKey
  messageMap: any;

  // LocalStorageKey
  keyMapLocalStorage: "keyMapRoomLocalStorage";
  keyListLocalStorage: "keyListRoomLocalStorage";
  ...
  // {from, message, room_id, from_last_name, from_first_name}
  addElem(message:any) {
    let isAdd = false;
    ...
    for (let key in this.keyList) {
      if (key.localeCompare(message.room_id) === 0) {
        console.log("key === message.room_id");
    ...
    if (!isAdd) {
      ...
      this.keyList.push(message.room_id);
      if (message.from.localeCompare(localStorage.getItem('userId')) === 0) {
  ...
  addToMap(message:any, isUser: boolean, isExist: boolean) {
    ...
    let elem;
    if (isExist) {
      elem = this.messageMap[message.room_id];
      if (isUser) {
      ...
      this.messageMap[message.room_id] = elem;
  ...
  public getListMessageByGroupId(room_id: string) {
    for (let key in this.keyList) {
      if (key.localeCompare(room_id) === 0) {
        return (this.messageMap[room_id]);
      }
    }
    return null;
  }
